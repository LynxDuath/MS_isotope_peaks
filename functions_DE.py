...
def get_inputs() -> (str, int, str, str):
    """
    Fragt nach den erforderlichen Eingaben.
    @return formula: str, Summenformel des Moleküls.
    @return charge: int, Ladung des Moleküls.
    @return precition: str, bevorzugte Präzision.
    @return graphics: str, bevorzugte graphische Ausgabe.
    """
    print("Dieses Programm unterstützt nur die folgenden Elemente:\nC, H, O, N, F, Cl, Br, I, S, P, Si, B\nZusätzlich:\nMe, Et, Bu, Ph, Bz, Ts, Ms und Klammern")
    formula = str(input("Bitte Summenformel eingeben: "))
    charge = input("Bitte Ladung eingeben: ")
    try:
        charge = int(charge)
    except ValueError:
        if charge[-1] == '-':
            ...
            charge = int(charge[:-1])
    precition = str(input("Möchten Sie das Ergebnis präzise oder unpräzise (u/p/up)? "))
    graphics = str(input("Möchten Sie graphische Ausgabe (Y/N)? "))
    print()
...
def get_inputs_inverted() -> list:
    """
    Fragt nach den erforderlichen Eingaben.
    @return Investigate: list, Verteilung des unbekannten Moleküls.
    """
    investigating = str(input("Welches Molekül soll untersucht werden? "))
...
def decipher_formula(formula: str, level: int, element, carbon=0, hydrogen=0, oxygen=0, nitrogen=0, fluorine=0, chlorine=0, bromine=0, iodine=0, sulfur=0, phosphorus=0, silicium=0, boron=0) -> array:
    """
    Dechiffriert die Summenformel.
    @param formula: str, Summenformel des Moleküls.
    @param level: int, Ebene der Summenformel, bezogen auf Klammern.
    @params carbon, hydrogen,...: int, Anzahl der Atome im Molekül zu Beginn, Standard 0.
    @returns cabon, hydrogen,...: int, Gesamtanzahl der Atome im Molekül.
    """
...
def distribute(Atomtype: list, atomtype: int, AtomtypeBase: list) -> list:
    """
    Berechnet die Verteilung der zusätzlichen Massen der gewählten Atomtyps.
    @param Atomtype: list, Verteilung des gewählten Atomtyps vor den Berechnungen, Standard [1].
    @param atomtype: int, Anzahl der Atome des gewählten Typs im Molekül.
    @param AtomtypeBase: list, Grundverteilung für ein Atom des gewählten Typs.
    @return Atomtype: list, Verteilung des gewählten Atomtyps nach der Berechnung.
    """
...
def norm(Molecule: list, maxprob: float, rounding: int, threshold: float, maxinten=100) -> list:
    """
    Normiert die Intensitätsverteilung.
    @param Molecule: list, Verteilung des Moleküls vor der Normierung.
    @param maxprob: float, maximale Wahrscheinlichkeit zur Normierung.
    @param rounding: int, Nachkommastelle zur Rundung der Intensitäten.
    @param threshold: float, Schwelle der Intensität zum Anzeigen der Peaks.
    @param maxinten: float, maximale Intensität für das Ergebnis der Normierung, Standard 100.
    @return MoleculeNorm: list, normierte Verteilung des Moleküls.
    """
    MoleculeNorm=[]
...
def graph(graphics: str, MoleculeNorm: list) -> None:
    ...
    @param graphics: str, bevorzugte graphische Ausgabe.
    @param MoleculeNorm: list, normierte Verteilung des Moleküls.
    """
    if graphics == 'Y':
        print("\n---Graphische Ausgabe---\n")
...
def print_inverted(mol_rms_min: float, best_molecule: list) -> None:
    """
    Gibt das erhaltene Molekül als Summenformel aus.
    @param mol_rms_min: float, minimale Abweichung der Isotopenverteilung von der Eingabe.
    @param best_molecule: list, Atomverteilung der besten Näherung.
    """
    output = 'Das Molekül ist '
    ...
    output += ' mit einer Übereinstimmung von ' + str((1-mol_rms_min)*100) + '%.'
    print(output)

#Zusätzlich
def wait():
    print("---Bitte haben Sie ein wenig Geduld!---\n")

def unprecition():
    sys.exit("Die Angabe war leider zu unpräzise!")
...
def calculate_unprecise(atoms: list, massmin: int, charge: int, CarbonBase, HydrogenBase, OxygenBase, NitrogenBase, FluorineBase, ChlorineBase, BromineBase, IodineBase, SulfurBase, PhosphorusBase, SiliciumBase, BoronBase, maxinten=100) -> list:
    """
    Berechnet die unpräzise Verteilung des Moleküls.
    @param atoms: list, Anzahl der Atome im Molekül.
    @param massmin: float, Masse des kleinsten Isotops des Moleküls.
    @param charge: int, Ladung des Moleküls.
    @param maxinten: float, maximale Intensität für das Ergebnis der Normierung, Standard 100.
    @return MoleculeNorm: list, normierte Verteilung des Moleküls. 
    """
    #Addition
...
def calculate_precise(atoms: list, massmin: float, charge: int, IsotopesMass: list, CarbonBase, HydrogenBase, OxygenBase, NitrogenBase, FluorineBase, ChlorineBase, BromineBase, IodineBase, SulfurBase, PhosphorusBase, SiliciumBase, BoronBase, maxinten=100) -> list:
    """
    Berechnet die präzise Verteilung des Moleküls.
    @param atoms: list, Anzahl der Atome im Molekül.
    @param massmin: float, Masse des kleinsten Isotops des Moleküls.
    @param charge: int, Ladung des Moleküls.
    @param IsotopesMass: list, Basisinformation der Isotopenmassen.
    @param maxinten: float, maximale Intensität für das Ergebnis der Normierung, Standard 100.
    @return MoleculeNorm: list, normierte Verteilung des Moleküls. 
    """
    #Verteilungen und Addition
...
def invert(Investigate: list, investigate_massmin: int, maxinten, CarbonBase, HydrogenBase, OxygenBase, NitrogenBase, FluorineBase, ChlorineBase, BromineBase, IodineBase, SulfurBase, PhosphorusBase, SiliciumBase, BoronBase) -> list:
    """
    Berechnet mögliche Zusammensetzungen eines unbekannten Moleküls mit einer gegebenen Massenverteilung.
    @param Investigate: list, Verteilung des unbekannten Moleküls.
    @param investigate_massmin: int: Masse des leichtesten Isotops des Moleküls.
    @param maxinten: float, maximale Intensität für das Ergebnis der Normierung.
    @return possible_molecules: list, Mögliche Zusammensetzungen des unbekannten Moleküls.
    """
    if len(Investigate) >1:
        if Investigate[1][1]/Investigate[0][1] >= 1:
            sys.exit("Das gewählte Molekül ist zu groß für dieses Programm, ein Cluster/Fulleren, eine Überschneidung von merheren Molekülen (z.B. Säure) oder enthält Bor.\nSo oder so, dieses Programm wird nicht funktionieren.")
